(() => {
  ...
  function openProfileDialog() {
    ...
    openModal({
      ...
      subtitle: signedIn ? "Elle signe vos messages dans tous vos chantiers." : guest ? "Un lien sécurisé sera envoyé à cette adresse e-mail." : "Elle apparaît sur les messages enregistrés sur cet appareil.",
      body: signedIn || !guest ? `<form id="profileForm" class="form-grid one"><label class="form-field">Nom et prénom<input name="full_name" required value="${escapeHtml(app.profile.full_name || "")}" placeholder="Ex. Yoann PETIT"></label><label class="form-field">Entreprise / équipe<input name="company" value="${escapeHtml(app.profile.company || "")}" placeholder="Ex. UO Travaux – SNCF Réseau"></label>${signedIn ? `<label class="form-field">E-mail<input disabled value="${escapeHtml(app.profile.email || app.user.email || "")}"></label>` : ""}</form>` : `<form id="authForm" class="form-grid one"><p class="form-note">Saisis l’adresse à laquelle tu as été invité. Après ouverture du lien reçu, l’accès aux chantiers partagés sera automatiquement chargé.</p><label class="form-field">E-mail professionnel<input name="email" type="email" required autocomplete="email" placeholder="prenom.nom@entreprise.fr"></label></form>`,
      footer: signedIn ? `<button class="secondary-button" id="signOutBtn">Se déconnecter</button><button class="primary-button" id="saveProfileBtn">Enregistrer</button>` : guest ? `<button class="secondary-button" id="closeProfileBtn">Annuler</button><button class="primary-button" id="sendMagicLinkBtn">Recevoir le lien</button>` : `<button class="secondary-button" id="closeProfileBtn">Fermer</button><button class="primary-button" id="saveProfileBtn">Enregistrer</button>`
    });
    if (signedIn || !guest) {
      ...
      $("sendMagicLinkBtn").addEventListener("click", async () => {
        ...
        try {
          ...
          if (!redirectTo || !redirectTo.startsWith("https://")) throw new Error("Publie d’abord l’application sur GitHub Pages en HTTPS avant d’envoyer un lien de connexion.");
          const { error } = await app.db.auth.signInWithOtp({ email, options: { emailRedirectTo: redirectTo } });
          if (error) throw error;
          toast("Lien de connexion envoyé. Ouvre-le sur cet appareil.", "success");
          closeModal();
  ...
  async function initialize() {
    wireEvents();
    if ("serviceWorker" in navigator && location.protocol !== "file:") navigator.serviceWorker.register("./service-worker-v4.js").catch(error => console.warn("Service worker", error));
    syncFromLocal();
    if (cloudConfigured()) {
