(() => {
  ...
  function attachmentUrl(attachment) {
    ...
    // Les aperçus restent stockés pour une future optimisation, mais ne doivent
    // jamais bloquer ou masquer une photo dans la discussion.
    if (fileIsImage(attachment)) return attachment.full_signed_url || attachment.signed_url || attachment.data_url || attachment.url || attachment.preview_signed_url || "";
  ...
  function markAttachmentImageLoaded(image) {
    ...
    const timer = Number(image.dataset.loadTimer || 0);
    if (timer) window.clearTimeout(timer);
    delete image.dataset.loadTimer;
    const holder = image.closest(".image-attachment");
    holder?.classList.remove("is-loading", "is-retrying", "is-unavailable", "is-stalled");
    image.dataset.loaded = "1";
    // La régénération d'aperçus d'anciennes photos est volontairement manuelle.
    // La lancer pendant l'affichage d'un fil peut saturer un téléphone et ne
    // doit jamais influencer la lecture de l'image originale.
  }

  // Les événements load/error délégués depuis le fil ne sont pas fiables dans
  // certains navigateurs Android. Chaque image reçoit donc ses propres
  // écouteurs, plus un contrôle de sécurité : une photo ne peut jamais rester
  // masquée derrière le libellé « Chargement » indéfiniment.
  function watchAttachmentImage(image) {
    if (!(image instanceof HTMLImageElement)) return;
    const previousTimer = Number(image.dataset.loadTimer || 0);
    ...
    const settle = () => {
      ...
      markAttachmentImageLoaded(image);
    };
    const fail = () => {
      if (image.dataset.loadToken !== token) return;
      recoverAttachmentImage(image);
    };
    image.onload = settle;
    image.onerror = fail;
    // decode() apporte un second signal fiable sur Chrome Android, même quand
    // l'événement load a été déclenché avant la pose de l'écouteur.
    if (typeof image.decode === "function") {
      image.decode().then(() => {
        if (image.dataset.loadToken === token && image.naturalWidth > 0) settle();
      }).catch(() => null);
    }
    const checkNow = () => {
      if (image.dataset.loadToken !== token || image.dataset.loaded === "1") return;
      if (image.complete && image.naturalWidth > 0) settle();
      else if (image.complete) fail();
    };
    queueMicrotask(checkNow);
    // Un délai de sécurité garantit qu'une photo ne reste jamais cachée derrière
    // un écran de chargement. L'original reçoit plus de temps sur réseau terrain.
    const timeout = image.dataset.source === "preview" ? 4000 : 12000;
    const timer = window.setTimeout(() => {
      if (!image.isConnected || image.dataset.loadToken !== token || image.dataset.loaded === "1") return;
      if (image.complete && image.naturalWidth > 0) return settle();
      image.closest(".image-attachment")?.classList.add("is-stalled");
      fail();
    }, timeout);
    image.dataset.loadTimer = String(timer);
  }
  ...
  async function recoverAttachmentImage(image) {
    ...
    if (retryCount >= 2) {
      holder?.classList.remove("is-loading", "is-retrying", "is-stalled");
      holder?.classList.add("is-unavailable");
      return;
    }
    app.imageRecoveryInFlight.add(attachmentId);
    image.dataset.retry = String(retryCount + 1);
    holder?.classList.remove("is-unavailable", "is-stalled");
    holder?.classList.add("is-loading", "is-retrying");
    const label = holder?.querySelector(".image-loading > span:last-child");
    if (label) label.textContent = retryCount === 0 ? "Nouvelle tentative de chargement…" : "Ouverture de l’original…";
    try {
      ...
      image.dataset.loaded = "";
      image.dataset.source = usePreview ? "preview" : "original";
      watchAttachmentImage(image);
  ...
  function renderMessages({ keepPosition = false } = {}) {
    ...
    requestAnimationFrame(() => $$(".attachment-image", els.messageFeed).forEach(watchAttachmentImage));
  ...
  async function initialize() {
    ...
    if ("serviceWorker" in navigator && location.protocol !== "file:") navigator.serviceWorker.register("./service-worker-v13.js?v=13.1").catch(error => console.warn("Service worker", error));
    syncFromLocal();
    if (cloudConfigured()) {
