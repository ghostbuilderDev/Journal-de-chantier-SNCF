(() => {
  ...
  const app = {
    ...
    // Les URLs Storage restent valables une heure. Les mémoriser évite de
    // remplacer toutes les balises <img> à chaque synchronisation temps réel.
    attachmentUrlCache: new Map(),
    photoViewer: { attachmentId: "", retryCount: 0, retrying: false },
  ...
  async function signedCloudAttachmentUrl(storagePath, { force = false } = {}) {
    ...
    // Une URL signée est valable une heure. Sans ce cache, chaque événement
    // temps réel régénérait toutes les URLs puis recréait les images du fil.
    // Sur Android, cette succession annulait parfois le décodage en cours.
    const cached = app.attachmentUrlCache.get(storagePath);
    ...
    try {
      ...
      if (url) app.attachmentUrlCache.set(storagePath, { url, expiresAt: Date.now() + 54 * 60 * 1000 });
      return url;
    } catch (error) {
  ...
  async function markChantierRead() {
    ...
    const latestMessageAt = Math.max(...app.messages.map(message => new Date(message.created_at || 0).getTime()).filter(Number.isFinite), 0);
    if (!latestMessageAt) return;
    const ownReadState = (app.readStates || []).find(state => String(state.user_id) === String(ownId()));
    // Aucun appel si l'état lu est déjà plus récent que le dernier message.
    // C'est la barrière qui évite la boucle : lecture -> temps réel -> refresh
    // -> lecture -> temps réel.
    if (ownReadState && new Date(ownReadState.last_read_at || 0).getTime() >= latestMessageAt) return;
    app.readReceiptInFlight = true;
    const timestamp = nowIso();
    try {
      const { error } = await app.db.from("chantier_read_states").upsert({
        chantier_id: app.currentId, user_id: ownId(), last_read_at: timestamp, updated_at: timestamp
      }, { onConflict: "chantier_id,user_id" });
      if (error) throw error;
      const nextState = { chantier_id: app.currentId, user_id: ownId(), last_read_at: timestamp, updated_at: timestamp };
      const existingIndex = app.readStates.findIndex(state => String(state.user_id) === String(ownId()));
      if (existingIndex >= 0) app.readStates.splice(existingIndex, 1, { ...app.readStates[existingIndex], ...nextState });
      else app.readStates.push(nextState);
    } finally {
      app.readReceiptInFlight = false;
    }
  }
  ...
  function subscribeCurrentChantier() {
    ...
    const channel = app.db.channel(`chantier-${app.currentId}`)
      ...
      // Une mise à jour de lecture émise par cet appareil ne doit pas relancer
      // tout le fil : sinon les photos sont démontées et rechargées en boucle.
  ...
  async function refreshAttachmentUrl(attachment, usePreview = false) {
    ...
    const signedUrl = await signedCloudAttachmentUrl(storagePath, { force: true });
    if (!signedUrl) throw new Error("Impossible de renouveler le lien de la photo.");
    rememberAttachmentUrl(attachment.id, signedUrl, usePreview);
    return signedUrl;
  }
  ...
  async function recoverFeedImage(image) {
    ...
    const attempts = Number(image.dataset.retry || 0);
    if (!attachmentId || attempts >= 1 || image.dataset.recovering === "1") {
      holder?.classList.add("is-unavailable");
      holder?.setAttribute("title", "Photo indisponible — toucher pour l’ouvrir ou la réessayer");
      return;
    }
    const attachment = findAttachment(attachmentId);
    if (!attachment) return;
    image.dataset.retry = String(attempts + 1);
  ...
  function setPhotoViewerStatus(text = "", state = "") {
    els.photoViewerStatus.textContent = text;
    els.photoViewerStatus.hidden = !text;
  ...
  function openPhotoViewer(attachment) {
    ...
    els.photoViewer.classList.remove("is-hidden");
  ...
  async function retryPhotoViewerImage() {
    ...
    const attachment = findAttachment(viewerState.attachmentId);
    if (!attachment || viewerState.retrying || viewerState.retryCount >= 1 || els.photoViewer.hidden) {
      if (!els.photoViewer.hidden) setPhotoViewerStatus("Photo indisponible. Utilise « Ouvrir » pour la télécharger.", "error");
      return;
    }
    viewerState.retrying = true;
    viewerState.retryCount += 1;
  ...
  async function openAttachment(attachment) {
    ...
    if (fileIsImage(attachment)) { openPhotoViewer(attachment); return; }
  ...
  async function handleDynamicClick(event) {
    ...
    if (action === "remove-pending") {
      ...
      // au navigateur mobile. Le visualiseur photo a son propre cycle de vie.
      event.preventDefault();
      event.stopPropagation();
      const attachment = findAttachment(element.dataset.attachmentId);
      if (attachment) openPhotoViewer(attachment);
  ...
  function wireEvents() {
    ...
    // Le visualiseur photo est isolé des fenêtres de formulaire. Il ne se
    ...
    els.photoViewerImage.addEventListener("error", () => { retryPhotoViewerImage(); });
