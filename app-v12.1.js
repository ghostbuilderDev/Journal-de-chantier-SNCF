(() => {
  ...
  function rememberAttachmentUrl(attachmentId, url) {
    app.messages.forEach(message => (message.attachments || []).forEach(attachment => {
      if (String(attachment.id) === String(attachmentId)) attachment.signed_url = url;
    }));
  }
  async function refreshAttachmentUrl(attachment) {
    const currentUrl = attachmentUrl(attachment);
    if (!isCloudReady() || !attachment?.storage_path) return currentUrl;
    ...
    rememberAttachmentUrl(attachment.id, data.signedUrl);
  ...
  async function recoverAttachmentImage(image) {
    ...
    try {
      const freshUrl = await refreshAttachmentUrl(attachment);
  ...
  function renderAttachment(attachment) {
    ...
    if (fileIsImage(attachment) && url) return `<button class="image-attachment" data-action="open-image" data-attachment-id="${attachment.id}" title="Agrandir la photo"><img class="attachment-image" loading="eager" decoding="async" src="${escapeHtml(url)}" alt="${escapeHtml(name)}" data-attachment-id="${attachment.id}"><span class="image-fallback">Photo indisponible — toucher pour réessayer</span></button>`;
  ...
  function openOwnerChantierMaintenanceDialog() {
    ...
    openModal({
      ...
      body: `<div class="form-note"><b>À utiliser avec prudence.</b><br>Tu peux effacer le fil sans supprimer le chantier, ou supprimer définitivement tout le chantier. Les fichiers stockés sont également supprimés.</div><label class="form-field">Chantier concerné<select id="ownerMaintenanceChantier">${options}</select></label>`,
      footer: `<button class="secondary-button" id="closeOwnerMaintenance">Fermer</button><button class="secondary-button" id="resetOwnerChantier">Réinitialiser le fil</button><button class="danger-button" id="deleteOwnerChantier">Supprimer le chantier</button>`,
      wide: false
    });
  ...
  async function openAttachment(attachment) {
    ...
    if (fileIsImage(attachment) && isCloudReady() && attachment.storage_path) {
      try { url = await refreshAttachmentUrl(attachment); }
  ...
  async function initialize() {
    ...
    if ("serviceWorker" in navigator && location.protocol !== "file:") navigator.serviceWorker.register("./service-worker-v12.1.js?v=12.1").catch(error => console.warn("Service worker", error));
    syncFromLocal();
    if (cloudConfigured()) {
