(() => {
  ...
  function openPasswordAuthDialog(view = "choice") {
    ...
    openModal({
      ...
      footer: `<button class="secondary-button" id="authBackBtn">Retour</button><button class="secondary-button" id="authDemoBtn">Voir la démo</button><button class="primary-button" id="authSubmitBtn">${creating ? "Créer mon accès" : "Se connecter"}</button>`
    });
  ...
  async function initialize() {
    ...
    if ("serviceWorker" in navigator && location.protocol !== "file:") navigator.serviceWorker.register("./service-worker-v9.js").catch(error => console.warn("Service worker", error));
    syncFromLocal();
    if (cloudConfigured()) {
