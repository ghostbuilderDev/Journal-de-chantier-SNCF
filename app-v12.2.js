(() => {
  ...
  const app = {
    ...
    chantiers: [], currentId: null, messages: [], actions: [], members: [], reactions: [], readStates: [], local: null,
  ...
  const els = {
    ...
    newChantierBtn: $("newChantierBtn"), inviteBtn: $("inviteBtn"), adminDashboardBtn: $("adminDashboardBtn"), notificationBtn: $("notificationBtn"),
  ...
  function defaultLocalData() {
    return { profile: { id: `local-${makeId()}`, full_name: "", company: "", email: "" }, chantiers: [], messages: [], actions: [], members: [], reactions: [], readStates: [], currentId: null };
  }
  ...
  async function hydrateCloudAttachments(messages) {
    ...
    const rows = await Promise.all((data || []).map(async attachment => {
      const isPhoto = fileIsImage(attachment);
      const [originalUrl, previewUrl] = await Promise.all([
        signedCloudAttachmentUrl(attachment.storage_path),
        isPhoto && attachment.preview_storage_path ? signedCloudAttachmentUrl(attachment.preview_storage_path) : Promise.resolve("")
      ]);
      return {
        ...attachment,
        // La vignette est volontairement utilisée dans le fil. L'original n'est
        // téléchargé qu'au toucher : le fil reste fluide sur réseau terrain.
        signed_url: originalUrl,
        full_signed_url: originalUrl,
        preview_signed_url: previewUrl
      };
    }));
  ...
  async function refreshCloudCurrent() {
    if (!isCloudReady() || !app.currentId) { app.messages = []; app.actions = []; app.reactions = []; app.readStates = []; renderAll({ keepPosition: true }); return; }
    const [messageResponse, actionResponse, reactionResponse, readResponse] = await Promise.all([
    ...
    // Les deux nouvelles tables sont optionnelles tant que le script V12 n’a
    // pas été exécuté : le journal reste utilisable, sans réactions/lectures.
  ...
  function attachmentUrl(attachment) {
    if (fileIsImage(attachment)) return attachment.preview_signed_url || attachment.full_signed_url || attachment.signed_url || attachment.data_url || attachment.url || "";
  ...
  function markAttachmentImageLoaded(image) {
    if (!(image instanceof HTMLImageElement)) return;
    const holder = image.closest(".image-attachment");
    holder?.classList.remove("is-loading", "is-retrying", "is-unavailable");
    image.dataset.loaded = "1";
    const attachment = findAttachment(image.dataset.attachmentId);
    // Un ancien original déjà chargé devient son propre aperçu réduit pour les
    // ouvertures suivantes. Cette réparation reste réservée au propriétaire.
    repairAttachmentPreview(attachment).catch(error => console.info("Réparation d’aperçu différée", error));
  }
  async function recoverAttachmentImage(image) {
    if (!(image instanceof HTMLImageElement)) return;
    const attachmentId = image.dataset.attachmentId;
    if (!attachmentId || app.imageRecoveryInFlight.has(attachmentId)) return;
    const holder = image.closest(".image-attachment");
    const attachment = findAttachment(attachmentId);
    if (!attachment) { holder?.classList.add("is-unavailable"); return; }
    const retryCount = Number(image.dataset.retry || 0);
    if (retryCount >= 2) {
      holder?.classList.remove("is-retrying");
      holder?.classList.add("is-unavailable");
      return;
    }
    app.imageRecoveryInFlight.add(attachmentId);
    image.dataset.retry = String(retryCount + 1);
    holder?.classList.add("is-retrying");
    try {
      // Premier essai : renouvelle la vignette. Si elle échoue une seconde fois,
      // le fil bascule automatiquement sur l’original au lieu de laisser un vide.
      const usePreview = Boolean(attachment.preview_storage_path && image.dataset.source !== "original" && retryCount === 0);
      if (!usePreview && attachment.preview_storage_path) image.dataset.source = "original";
      let freshUrl = "";
      try {
        freshUrl = await refreshAttachmentUrl(attachment, usePreview);
      } catch (previewError) {
        if (!usePreview) throw previewError;
        freshUrl = await refreshAttachmentUrl(attachment, false);
        image.dataset.source = "original";
      }
      if (!freshUrl) throw new Error("Photo indisponible.");
      image.src = `${freshUrl}${freshUrl.includes("?") ? "&" : "?"}retry=${Date.now()}`;
    } catch (error) {
      console.warn("Photo indisponible", error);
      holder?.classList.remove("is-retrying");
      holder?.classList.add("is-unavailable");
    } finally {
      app.imageRecoveryInFlight.delete(attachmentId);
    }
  }
  ...
  function renderAttachment(attachment) {
    ...
    if (fileIsImage(attachment) && url) {
      const source = attachment.preview_storage_path && attachment.preview_signed_url ? "preview" : "original";
      return `<button class="image-attachment is-loading" data-action="open-image" data-attachment-id="${attachment.id}" title="Agrandir la photo"><span class="image-loading" aria-live="polite"><span class="image-spinner" aria-hidden="true"></span><span>Chargement de l’aperçu…</span></span><img class="attachment-image" loading="eager" decoding="async" fetchpriority="high" src="${escapeHtml(url)}" alt="${escapeHtml(name)}" data-attachment-id="${attachment.id}" data-source="${source}"><span class="image-fallback">Photo indisponible — toucher pour réessayer</span></button>`;
    }
  ...
  function renderMessages({ keepPosition = false } = {}) {
    ...
    requestAnimationFrame(() => {
      $$(".attachment-image", els.messageFeed).forEach(image => {
        if (image.complete && image.naturalWidth > 0) markAttachmentImageLoaded(image);
      });
    });
  ...
  function renderAll(options) { renderProfile(); renderConnection(); renderAccessControls(); renderSidebar(); renderHeader(); renderPinnedMessages(); renderMessages(options); renderPlans(); renderActions(); renderPrintCover(); }
  ...
  async function uploadCloudAttachment(file, message, metadata = {}) {
    ...
    if (storageError) throw storageError;
    let previewPath = "";
    const previewFile = await createPhotoPreviewFile(file);
    if (previewFile) {
      const candidate = `${path}.preview.jpg`;
      const { error: previewError } = await bucket.upload(candidate, previewFile, { contentType: "image/jpeg", upsert: false });
      if (previewError) console.warn("Aperçu photo non envoyé", previewError);
    ...
    if (previewPath) attachment.preview_storage_path = previewPath;
    let { data, error } = await app.db.from("chantier_attachments").insert(attachment).select().single();
    // L'application reste utilisable si le fichier SQL V12.2 n'a pas encore été
    // exécuté : l'original est conservé, seul l'aperçu optimisé est différé.
    if (error && previewPath && previewColumnUnavailable(error)) {
      await bucket.remove([previewPath]).catch(() => null);
      delete attachment.preview_storage_path;
      ({ data, error } = await app.db.from("chantier_attachments").insert(attachment).select().single());
    }
    if (error) throw error;
    return data;
  }
  ...
  function beginPasswordRecovery(user) {
    ...
    app.mode = "recovery";
    app.user = user;
    app.chantiers = []; app.currentId = null; app.messages = []; app.actions = []; app.members = [];
    renderAll();
  ...
  function wireEvents() {
    ...
    [els.messageFeed, els.attachmentPreview, els.replyPreview, els.planGrid, els.actionBoard, els.pinnedMessages].forEach(target => target.addEventListener("click", handleDynamicClick));
    els.messageFeed.addEventListener("load", event => {
      if (event.target instanceof HTMLImageElement && event.target.classList.contains("attachment-image")) markAttachmentImageLoaded(event.target);
    }, true);
    els.messageFeed.addEventListener("error", event => {
      if (event.target instanceof HTMLImageElement && event.target.classList.contains("attachment-image")) recoverAttachmentImage(event.target);
    }, true);
  ...
  async function initialize() {
    ...
    if ("serviceWorker" in navigator && location.protocol !== "file:") navigator.serviceWorker.register("./service-worker-v12.2.js?v=12.2").catch(error => console.warn("Service worker", error));
    syncFromLocal();
    if (cloudConfigured()) {
