...
(async()=>{
 ...
 try{
  ...
  await setup159(db);await db.exec('create table if not exists public.journal_sql_migrations(migration_name text primary key,applied_at timestamptz default now());');
  const sql=execFileSync(process.env.PYTHON||'python3',['scripts/deploy-v15101-release.py','--print-sql'],{cwd,encoding:'utf8'});
  await db.exec(sql);await db.exec(fs.readFileSync(path.join(__dirname,'reports-v1510-postgres.sql'),'utf8'));await db.exec(fs.readFileSync(path.join(__dirname,'collaboration-v15101-postgres.sql'),'utf8'));
...
spec=importlib.util.spec_from_file_location('release','scripts/deploy-v15101-release.py')
...
def query(sql,*args):
 queries.append(sql)
 return [{'ready':True}]
m.previous.briefing.transport.postgres_environment=lambda *args:None
m.previous.briefing.transport.postgres_query=query
...
`;
  ...
  console.log('V15.10.1 cumulative deployment OK: real SQL envelope, ledger and every installer readiness query.');
