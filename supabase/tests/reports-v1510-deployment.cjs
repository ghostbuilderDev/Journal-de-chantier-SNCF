...
(async()=>{
 ...
 try{
  ...
  const sql=execFileSync(process.env.PYTHON||'python3',['scripts/deploy-v1510-release.py','--print-sql'],{cwd,encoding:'utf8'});
  await db.exec(sql);await db.exec(fs.readFileSync(path.join(__dirname,'reports-v1510-postgres.sql'),'utf8'));
...
spec=importlib.util.spec_from_file_location('release','scripts/deploy-v1510-release.py')
...
`;
  ...
  console.log('V15.10 deployment OK: real SQL envelope, ledger and every installer readiness query.');
