// A populated V15.3 report stays intact through V15.4, including frozen emails.
const fs=require('node:fs'),path=require('node:path'),assert=require('node:assert/strict');
const {PGlite}=require(process.env.PGLITE_MODULE||'@electric-sql/pglite');const{setup153}=require('./cr-v153-backend.cjs');
const read=f=>fs.readFileSync(path.resolve(__dirname,'../..',f),'utf8');
(async()=>{const db=new PGlite();try{
 await setup153(db);const u='00000000-0000-4000-8000-000000000001',site='aaaaaaaa-0000-4000-8000-000000000001';
 ...
 for(let i=0;i<2;i++)await db.exec(read('supabase/migrations/20260909000200_v15_4_production_email.sql'));
 ...
 const {emailMarkup}=await import('../functions/_shared/cr-email.mjs');assert.match(emailMarkup(updatedMail),/<table/);assert.ok(emailMarkup(updatedMail).includes(new Intl.DateTimeFormat('fr-FR',{hour:'2-digit',minute:'2-digit',hourCycle:'h23',timeZone:'Europe/Paris'}).format(new Date(night+'T20:10Z'))));
 console.log('PASS V15.4 retrofit: actual hours, labels, authors, recipients and immutable snapshots retained; old validated email byte-identical; new ARF planned/actual formatted by the existing sender.');
