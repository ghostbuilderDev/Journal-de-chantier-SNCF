...
const sql=execFileSync(process.env.PYTHON||'python3',[path.join(root,'scripts/deploy-v155-release.py'),'--print-sql'],{encoding:'utf8'});
(async()=>{const db=new PGlite();try{
 ...
 assert.equal((await db.query("select to_regclass('journal_cr_private.field_requests')::text name")).rows[0].name,null);
 ...
 await db.exec(sql);
 assert.equal((await db.query("select count(*)::int n from public.journal_sql_migrations where migration_name='20260909000300_v15_5_requests_pdf.sql'")).rows[0].n,1);
 assert.ok((await db.query("select to_regclass('journal_cr_private.field_requests')::text name")).rows[0].name);
 assert.equal((await db.query('select count(*)::int n from public.chantier_messages')).rows[0].n,originalMessages);
 console.log('PASS V15.5 installer: exact wrapped transaction executes, failure rolls back SQL and ledger, prior API retained on failure, successful migration registered once, messages retained.');
