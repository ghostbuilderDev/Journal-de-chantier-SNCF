...
(async()=>{
 ...
 try{
  ...
  await setup156(db);await db.exec('create table if not exists public.journal_sql_migrations(migration_name text primary key,applied_at timestamptz default now());');
  const sql=execFileSync(process.env.PYTHON||'python3',['scripts/deploy-v157-release.py','--print-sql'],{cwd,encoding:'utf8'});
  await db.exec(sql);await db.exec(fs.readFileSync(path.join(__dirname,'cr-v157-postgres.sql'),'utf8'));
...
spec=importlib.util.spec_from_file_location('release','scripts/deploy-v157-release.py')
...
`;
  ...
  console.log('V15.7 deployment OK: real SQL envelope, ledger and every installer readiness query.');
