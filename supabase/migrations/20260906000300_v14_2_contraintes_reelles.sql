...
begin
  for r in select * from (values
    ...
    ('journal_access_requests','id','uuid'),('journal_access_requests','requester_id','uuid'),
    ('journal_access_requests','status','text'),('journal_access_requests','requested_at','timestamp with time zone'),
...
-- Diagnostic réel du 6 septembre : la CHECK body/deleted_at autorise déjà ''.
-- La conserver intégralement, y compris sa protection des messages non supprimés.
-- Les anciennes CHECK strictement mono-colonne restent compatibles aussi.
do $$
declare r record;
begin
  for r in select c.conname,cardinality(c.conkey) as arity,pg_get_expr(c.conbin,c.conrelid) as expression,
    ...
    if r.arity=2 and r.definition='CHECK (body IS NOT NULL OR deleted_at IS NOT NULL)' then
      continue; -- Aucune suppression, réécriture ou recréation de cette CHECK.
    end if;
...
begin
  for r in select c.oid,c.relname from pg_class c join pg_namespace n on n.oid=c.relnamespace
    where n.nspname='public' and c.relkind='r' and c.relname in (
      ...
      'chantier_read_states','chantier_document_folders','chantier_documents','journal_portal_apps','chantier_invitations') loop
...
begin
  ...
  select coalesce(jsonb_agg(rowdata order by lower(rowdata->>'full_name')),'[]'::jsonb) into result from (
    select jsonb_build_object('id',u.id,'user_id',u.id,'email',coalesce(p.email,u.email),'full_name',
      ...
      'request_status',case when b.user_id is not null then 'refusee' else coalesce((select q.status
        from public.journal_access_requests q where q.requester_id=u.id
        order by q.requested_at desc nulls last,q.id desc limit 1),'acceptee') end,
...
begin
  ...
  update public.journal_access_requests set status='acceptee' where requester_id=p_user_id;
...
begin
  ...
  delete from public.chantier_members where user_id=p_user_id;
  delete from public.journal_administrators where user_id=p_user_id;
  update public.journal_access_requests set status='refusee' where requester_id=p_user_id;
end $$;
...
begin
  for r in select c.oid,c.conname,c.conrelid,c.confrelid,c.conkey,c.confkey,c.confdeltype,
    ...
    if (r.table_name='profiles' and r.column_name='id') or
      (r.table_name in ('chantier_members','journal_administrators','chantier_read_states','chantier_message_reactions') and r.column_name='user_id') or
      (r.table_name='journal_access_requests' and r.column_name='requester_id') then
    ...
    v_allowed:=(r.table_name in ('chantiers','chantier_messages','chantier_attachments','action_items','chantier_daily_logs','chantier_risks','chantier_documents','chantier_document_folders','journal_portal_apps')
      ...
      -- L'invitation est conservée ; seul le lien vers son auteur supprimé est détaché.
      or (r.table_name='chantier_invitations' and r.column_name='invited_by');
...
begin
  ...
  for r in select c.conname,c.confdeltype,n.nspname,t.relname,a.attname,cardinality(c.conkey) as arity
    ...
    if r.nspname='public' and r.arity=1 and ((r.relname='profiles' and r.attname='id') or
      (r.relname in ('chantier_members','journal_administrators','chantier_read_states','chantier_message_reactions') and r.attname='user_id') or
      (r.relname='journal_access_requests' and r.attname='requester_id')) then continue; end if;
  ...
  delete from public.chantier_members where user_id=p_user_id;
  delete from public.journal_administrators where user_id=p_user_id;
  delete from public.journal_access_requests where requester_id=p_user_id;
