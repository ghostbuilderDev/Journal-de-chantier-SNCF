...
prepare() { python "$RELEASE_ROOT/scripts/prepare-v157-update.py" "$1" "$REPO_DIR"; }
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-V15.7-sauvegarde-XXXXXXXX")"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.7 - catalogue SEL secteurs et ZEP'
  git_remote push origin main
...
echo 'Attente : tests des droits et mise a jour du catalogue V15.7...'
python -u "$RELEASE_ROOT/scripts/wait-v157-workflow.py" "$backend_sha" "${wait_options[@]}"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.7 - horaires natifs et intitules simplifies'
  git_remote push origin main
fi
PHASE='terminee'
echo 'Les donnees et les notifications existantes sont conservees.'
echo 'V15.7 envoyee : heures natives, dates sur demande, intitule unique et catalogue complete.'
