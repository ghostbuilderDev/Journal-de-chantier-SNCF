...
prepare() { python "$RELEASE_ROOT/scripts/prepare-v153-update.py" "$1" "$REPO_DIR"; }
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-V15.3-sauvegarde-XXXXXXXX")"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.3 - tableaux prevus et reels'
  git_remote push origin main
...
echo 'Attente : tests des droits, migration V15.3 et configuration des notifications...'
python -u "$RELEASE_ROOT/scripts/wait-v153-workflow.py" "$backend_sha" "${wait_options[@]}"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.3 - actions retours CR et photos PDF'
  git_remote push origin main
fi
PHASE='terminee'
echo 'Les reglages Supabase, email et notifications existants sont conserves.'
echo 'V15.3 envoyee : actions vertes, signalements, tableaux CR et photos dans les PDF.'
...
echo 'Ouvrez CR off, puis Consignation, ITC ou ARF : responsable et tableau dans une seule fenetre. Les agents retrouvent Mes demandes.'
