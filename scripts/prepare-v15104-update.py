#!/usr/bin/env python3
"""Bounded V15.10.3 -> V15.10.4, backend checked before the interface."""
import hashlib,json,subprocess,sys
from pathlib import Path
sys.dont_write_bytecode=True
ROOT=Path(__file__).resolve().parents[1]
BACKEND=['supabase/migrations/20260910000500_v15_10_4_report_numbers.sql', 'supabase/tests/report-numbering-fixture.sql', 'supabase/tests/report-numbering-v15104-backend.cjs', 'supabase/tests/report-numbering-v15104-deployment.cjs', 'supabase/tests/report-numbering-v15104-postgres.sql', 'scripts/deploy-v15104-release.py', '.github/workflows/deploy-v15104.yml', 'scripts/prepare-v15104-update.py', 'scripts/update-v15104-termux.sh', 'scripts/wait-v15104-workflow.py', 'scripts/v15104-base-sha256.json', 'tests/v15104-installation.test.py']
FRONTEND=['rapport/app.js', 'rapport/collaboration.js', 'rapport/index.html', 'rapport/journal-archive.js', 'rapport/service-worker.js', 'tests/collaboration-v15101-browser.cjs', 'tests/report-numbering-v15104-browser.cjs', 'tests/pwa-v15104-browser.cjs', 'VERSION_V15.10.4.md', 'VERIFICATIONS_V15.10.4.txt']
MESSAGES={'backend':'Journal Chantier V15.10.4 - numerotation des rapports','frontend':'Journal Chantier V15.10.4 - numeros uniques et reprise des rapports'}
...
def validate_change(name,current,proposed,expected,previous=()):
    ...
    if current!=proposed and digest(current)!=expected and digest(current) not in previous:
        raise ValueError(name+' ne correspond pas à la V15.10.3 vérifiée. Les fichiers sont conservés ; ce correctif doit être adapté à votre version.')

def prepare(repo):
    manifest=json.loads((ROOT/'scripts/v15104-base-sha256.json').read_text());changes={}
