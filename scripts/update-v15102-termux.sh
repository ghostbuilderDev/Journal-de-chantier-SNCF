...
prepare() { python "$RELEASE_ROOT/scripts/prepare-v15102-update.py" "$1" "$REPO_DIR"; }
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-V15.10.2-sauvegarde-XXXXXXXX")"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.10.2 - serveur cumulatif verifie'
  git_remote push origin main
...
echo 'Attente : tests des droits et rapports transferables et mentions V15.10.2...'
python -u "$RELEASE_ROOT/scripts/wait-v15102-workflow.py" "$backend_sha" "${wait_options[@]}"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.10.2 - rapprochement des signatures QR'
  git_remote push origin main
fi
PHASE='terminee'
echo 'Les donnees et les notifications existantes sont conservees.'
echo 'V15.10.2 envoyee : noms conserves, signatures QR rattachees et verification des cas ambigus.'
