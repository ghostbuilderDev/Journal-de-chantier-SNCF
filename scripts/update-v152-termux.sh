...
prepare() { python "$RELEASE_ROOT/scripts/prepare-v152-update.py" "$1" "$REPO_DIR"; }
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-V15.2-sauvegarde-XXXXXXXX")"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.2 - demandes individuelles et rappels'
  git_remote push origin main
...
echo 'Attente : tests des droits, migration V15.2 et configuration des notifications...'
python -u "$RELEASE_ROOT/scripts/wait-v152-workflow.py" "$backend_sha" "${wait_options[@]}"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.2 - CR off simplifie et Gemini'
  git_remote push origin main
fi
PHASE='terminee'
echo 'Pour le premier envoi email, consulter VERSION_V15.2.md (adresse expediteur a configurer).'
echo 'V15.2 envoyee : tableaux, demandes individuelles, rappels et Gemini.'
...
echo 'Ouvrez CR off puis Configurez les lignes et le responsable. Les agents retrouveront Mes demandes apres reouverture de leur application.'
