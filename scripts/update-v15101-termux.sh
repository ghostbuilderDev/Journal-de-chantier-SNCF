...
prepare() { python "$RELEASE_ROOT/scripts/prepare-v15101-update.py" "$1" "$REPO_DIR"; }
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-V15.10.1-sauvegarde-XXXXXXXX")"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.10.1 - serveur cumulatif et alertes'
  git_remote push origin main
...
echo 'Attente : tests des droits et rapports transferables et mentions V15.10.1...'
python -u "$RELEASE_ROOT/scripts/wait-v15101-workflow.py" "$backend_sha" "${wait_options[@]}"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.10.1 - signatures mobiles et collaboration'
  git_remote push origin main
fi
PHASE='terminee'
echo 'Les donnees et les notifications existantes sont conservees.'
echo 'V15.10.1 envoyee : rapports transferables, alertes immediates et signatures QR agrandies.'
