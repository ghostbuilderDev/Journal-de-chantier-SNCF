...
prepare() { python "$RELEASE_ROOT/scripts/prepare-v15105-update.py" "$1" "$REPO_DIR"; }
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-V15.10.5-sauvegarde-XXXXXXXX")"
...
PHASE='preparation du menu et de la numerotation partagee'
prepare backend
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.10.5 - menu flottant des actions du rapport'
  git_remote push origin main
fi
PHASE='terminee'
echo 'Les donnees et les notifications existantes sont conservees.'
echo 'V15.10.5 envoyee : Actions du rapport flottantes et numerotation partagee. Le Rapport journalier affiche V10.5.'
