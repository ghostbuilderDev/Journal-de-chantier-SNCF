...
def wait_for_workflow(sha, *, resume=False, api=gh_api, sleep=time.sleep,
    ...
    while now() < deadline:
        try:
            ...
            if resume and not dispatch_attempted:
                ...
                if workflow.get('state') != 'active' or not isinstance(workflow.get('id'), int):
                    raise RuntimeError('Le workflow V15.6 est desactive ou non valide dans GitHub. '
        ...
        if attempt % 6 == 0:
            output('Attente de l execution V15.6 dans GitHub...')
        attempt += 1
        sleep(5)
    if selected is None:
        diagnose_missing(sha, api, output)
        raise RuntimeError('Aucune execution V15.6 confirmee pour ce commit. '
    ...
    while now() < deadline:
        ...
        if status == 'completed':
            if selected.get('conclusion') == 'success':
                output('Serveur V15.6 valide par GitHub Actions.')
                return run_id
            raise RuntimeError(f'Le workflow V15.6 a termine avec : {selected.get("conclusion") or "resultat inconnu"}. '
                               f'Interface conservee. Diagnostic : gh run view {run_id} --repo {REPOSITORY} --log-failed')
        if status != previous_state:
            output('Journal V15.6 : ' + str(status or 'en attente') + '...')
...
def main():
    ...
    if not args or len(args) > 2 or len(args) == 2 and args[1] != '--resume':
        raise RuntimeError('Usage : wait-v156-workflow.py SHA [--resume]')
