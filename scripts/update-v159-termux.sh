...
prepare() { python "$RELEASE_ROOT/scripts/prepare-v159-update.py" "$1" "$REPO_DIR"; }
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-V15.9-sauvegarde-XXXXXXXX")"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.9 - completude partagee et droits'
  git_remote push origin main
...
echo 'Attente : tests des droits et completude partagee et partage QR V15.9...'
python -u "$RELEASE_ROOT/scripts/wait-v159-workflow.py" "$backend_sha" "${wait_options[@]}"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.9 - completude multi-agents et partage QR'
  git_remote push origin main
fi
PHASE='terminee'
echo 'Les donnees et les notifications existantes sont conservees.'
echo 'V15.9 envoyee : completude partagee Production / Securite et partage du QR de briefing.'
