...
prepare() { python "$RELEASE_ROOT/scripts/prepare-v15-update.py" "$1" "$REPO_DIR"; }
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-V15-sauvegarde-XXXXXXXX")"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15 - serveur CR encadrement et notifications'
  git_remote push origin main
...
echo 'Attente : tests des droits, migration V15 et configuration des notifications...'
python -u "$RELEASE_ROOT/scripts/wait-v15-workflow.py" "$backend_sha" "${wait_options[@]}"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15 - CR off et ameliorations terrain'
  git_remote push origin main
fi
PHASE='terminee'
echo 'Pour le premier envoi email, consulter VERSION_V15.md (adresse expediteur a configurer).'
echo 'V15 envoyee : CR prives, notifications et ameliorations terrain.'
...
echo 'Ouvrez CR off pour preparer une nuit. Dans Alertes, activez les notifications sur votre telephone.'
