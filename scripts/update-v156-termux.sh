...
prepare() { python "$RELEASE_ROOT/scripts/prepare-v156-update.py" "$1" "$REPO_DIR"; }
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-V15.6-sauvegarde-XXXXXXXX")"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.6 - references et suppressions definitives'
  git_remote push origin main
...
echo 'Attente : tests des droits, migration V15.6 et configuration des notifications...'
python -u "$RELEASE_ROOT/scripts/wait-v156-workflow.py" "$backend_sha" "${wait_options[@]}"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.6 - redaction agrandie et CR simplifies'
  git_remote push origin main
fi
PHASE='terminee'
echo 'Les donnees et les notifications existantes sont conservees.'
echo 'V15.6 envoyee : redaction agrandie, brouillons conserves, choix SEL / secteurs / ZEP et suppressions definitives.'
...
echo 'Pour Gemini Pro : lire VERSION_V15.6.md. Le service existant reste utilise tant que le moteur Pro n est pas active.'
