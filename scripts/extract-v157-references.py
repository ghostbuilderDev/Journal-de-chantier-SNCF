#!/usr/bin/env python3
"""V15.7 source extraction (openpyxl). Read-only workbook; no invented subdivisions."""
...
entries={};excluded=[];rows=0;direct=0;supplementary=0
for sheet in load_workbook(SOURCE,data_only=True):
 for row in range(5,sheet.max_row+1):
  owner=bool(re.search(r'\b(?:PETIT|DAHMANI|DAMANI)\b',str(sheet[f'R{row}'].value or ''),re.I))
  site=clean(str(sheet[f'S{row}'].value or ''))
  # The complementary Samois operation is explicitly present in this workbook.
  # Retain the exact grouped S9/S11 labels; do not infer individual isolations.
  related=fold(site)=='moret (samois)'
  if not (owner or related):continue
  rows+=1;direct+=int(owner);supplementary+=int(not owner)
...
data={'source_file':SOURCE.name,'scope':'Toutes les feuilles du classeur ; lignes Petit / Dahmani / Damani et compléments du chantier Moret (Samois). Regroupements S9/S11 conservés tels que fournis. Ni horaires ni numéros de téléphone importés.','source_sha256':hashlib.sha256(SOURCE.read_bytes()).hexdigest(),'matched_rows':rows,'direct_rows':direct,'supplementary_rows':supplementary,'entries':sorted(entries.values(),key=lambda e:(e['section_key'],e['type'],fold(e['label']))),'excluded_notes':excluded}
(ROOT/'data/cr-references-v157.json').write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n');print('Source rows:',rows,'References:',len(entries))
