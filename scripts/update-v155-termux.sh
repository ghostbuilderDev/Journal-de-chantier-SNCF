...
prepare() { python "$RELEASE_ROOT/scripts/prepare-v155-update.py" "$1" "$REPO_DIR"; }
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-V15.5-sauvegarde-XXXXXXXX")"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.5 - demandes privees et validation'
  git_remote push origin main
...
echo 'Attente : tests des droits, migration V15.5 et configuration des notifications...'
python -u "$RELEASE_ROOT/scripts/wait-v155-workflow.py" "$backend_sha" "${wait_options[@]}"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.5 - PDF et saisies terrain'
  git_remote push origin main
fi
PHASE='terminee'
echo 'Les donnees et les notifications existantes sont conservees.'
echo 'V15.5 envoyee : PDF avec photos, CR reserves aux administrateurs, brouillons et envoi definitif des saisies.'
...
echo 'Dans le CR : Visualiser le PDF, puis Partager le CR off. Aucun service email a configurer.'
