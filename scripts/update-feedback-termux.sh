...
prepare() { python "$RELEASE_ROOT/scripts/prepare-feedback-update.py" "$1" "$REPO_DIR"; }
...
for required in SUPABASE_PROJECT_ID SUPABASE_DB_URL; do
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-retours-V14.5-sauvegarde-XXXXXXXX")"
...
PHASE='publication du serveur des retours application'
prepare backend
git add -- "${backend_files[@]}"
wait_options=()
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V14.5 - serveur des retours application'
  git_remote push origin main
...
PHASE='validation du fil de retours avant publication de l interface'
echo 'Attente : tests des droits, migration V14.5 et verification du fil de retours...'
python -u "$RELEASE_ROOT/scripts/wait-feedback-workflow.py" "$backend_sha" "${wait_options[@]}"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V14.5 - fil des ameliorations application'
  git_remote push origin main
fi
PHASE='terminee'
echo 'V14.5 envoyee : fil des ameliorations et droits Supabase valides.'
...
echo 'Ouvrez le bouton Ameliorer l application pour partager une idee ou signaler un probleme.'
