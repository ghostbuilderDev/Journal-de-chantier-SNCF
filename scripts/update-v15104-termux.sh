...
prepare() { python "$RELEASE_ROOT/scripts/prepare-v15104-update.py" "$1" "$REPO_DIR"; }
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-V15.10.4-sauvegarde-XXXXXXXX")"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.10.4 - numerotation des rapports'
  git_remote push origin main
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.10.4 - numeros uniques et reprise des rapports'
  git_remote push origin main
fi
PHASE='terminee'
echo 'Les donnees et les notifications existantes sont conservees.'
echo 'V15.10.4 envoyee : numeros uniques des rapports et conservation des brouillons.'
