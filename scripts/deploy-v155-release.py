...
NAME='20260909000300_v15_5_requests_pdf.sql'
def migration_transaction():
 try:
  return briefing.transport.build_transaction((ROOT/'supabase/migrations'/NAME).read_text(),NAME)
 except ValueError as error:
  raise RuntimeError('Fichier SQL V15.5 invalide : '+str(error)+' Télécharger le paquet corrigé ; aucune migration exécutée.') from None
...
def main():
 ...
 if not briefing.is_ready(query("select to_regprocedure('journal_cr_private.api_v153(text,jsonb)') is not null and to_regclass('journal_cr_private.production_sheets') is not null and to_regclass('journal_cr_private.week_plans') is not null and to_regprocedure('journal_cr_private.api_v151(text,jsonb)') is not null and to_regprocedure('public.journal_feedback_context()') is not null and to_regprocedure('cron.schedule(text,text,text)') is not null as ready")):
  raise RuntimeError('V15.4, le service des signalements et le planificateur doivent être installés. Aucune ancienne migration rejouée.')
 ...
 print('V15.5 installée : demandes privées, enregistrement provisoire et envoi définitif, suppression récupérable, PDF manuel. Aucun email envoyé.')
