...
prepare() { python "$RELEASE_ROOT/scripts/prepare-v154-update.py" "$1" "$REPO_DIR"; }
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-V15.4-sauvegarde-XXXXXXXX")"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.4 - production partagee et email'
  git_remote push origin main
...
echo 'Attente : tests des droits, migration V15.4 et configuration des notifications...'
python -u "$RELEASE_ROOT/scripts/wait-v154-workflow.py" "$backend_sha" "${wait_options[@]}"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.4 - actions uniques CR et impression'
  git_remote push origin main
fi
PHASE='terminee'
echo 'Les donnees et les notifications existantes sont conservees.'
echo 'V15.4 envoyee : action unique, epingle 3 minutes, production partagee, apercu email et export optimise.'
...
echo 'Premier envoi de test : configurer une fois Resend avec la commande suivante.'
echo "bash '$REPO_DIR/scripts/configure-v154-email-termux.sh'"
