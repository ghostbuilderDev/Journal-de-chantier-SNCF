...
NAME='20260909000500_v15_7_reference_catalogue.sql'
def migration_transaction():
 try:
  return briefing.transport.build_transaction((ROOT/'supabase/migrations'/NAME).read_text(),NAME)
 except ValueError as error:
  raise RuntimeError('Fichier SQL V15.7 invalide : '+str(error)+' Télécharger le paquet corrigé ; aucune migration exécutée.') from None
...
def main():
 ...
 if not briefing.is_ready(query("select to_regprocedure('journal_cr_private.api_v155(text,jsonb)') is not null and to_regclass('journal_cr_private.references_v156') is not null and to_regclass('journal_cr_private.field_requests') is not null and to_regclass('journal_cr_private.production_sheets') is not null and to_regclass('journal_cr_private.week_plans') is not null and to_regprocedure('journal_cr_private.api_v151(text,jsonb)') is not null and to_regprocedure('public.journal_feedback_context()') is not null and to_regprocedure('cron.schedule(text,text,text)') is not null as ready")):
  raise RuntimeError('V15.6, le service des signalements et le planificateur doivent être installés. Aucune ancienne migration rejouée.')
 ...
 if not briefing.is_ready(query("select (select count(*)>=64 from journal_cr_private.references_v156) and not has_function_privilege('authenticated','journal_cr_private.api_v155(text,jsonb)','EXECUTE') and has_function_privilege('authenticated','public.journal_delete_site_v156(uuid)','EXECUTE') and not has_function_privilege('anon','public.journal_delete_site_v156(uuid)','EXECUTE') as ready")):
  raise RuntimeError('Catalogue ou droits V15.7 non validés.')
 print('V15.7 installée : catalogue SEL / secteurs / ZEP complété. Les CR, les horaires et les demandes existantes sont conservés.')
