#!/usr/bin/env python3
"""Bounded V15 -> V15.1 installation, repeatable backend-first phases."""
import hashlib,json,subprocess,sys
from pathlib import Path
sys.dont_write_bytecode=True
ROOT=Path(__file__).resolve().parents[1]
BACKEND=['.github/workflows/deploy-v151.yml', 'supabase/migrations/20260908000200_v15_1_cr_perimetres.sql', 'supabase/tests/cr-v151-backend.cjs', 'supabase/tests/cr-v151-postgres.sql', 'scripts/deploy-v151-release.py', 'scripts/prepare-v151-update.py', 'scripts/wait-v151-workflow.py', 'scripts/update-v151-termux.sh', 'scripts/v151-base-sha256.json', 'tests/v151-installation.test.py']
FRONTEND=['cr-off.js', 'cr-off.css', 'index.html', 'service-worker-v13.js', 'tests/cr-v151-browser.cjs', 'VERSION_V15.1.md', 'VERIFICATIONS_V15.1.txt']
MESSAGES={'backend': 'Journal Chantier V15.1 - serveur des horaires par perimetre', 'frontend': 'Journal Chantier V15.1 - CR off terrain et destinataires'}
...
def prepare(repo):
    manifest=json.loads((ROOT/'scripts/v151-base-sha256.json').read_text());changes={}
