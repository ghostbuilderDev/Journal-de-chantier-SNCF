...
prepare() { python "$RELEASE_ROOT/scripts/prepare-v151-update.py" "$1" "$REPO_DIR"; }
...
prepare check
import_file="$RELEASE_ROOT/donnees-privees/import-cr-v151.json"
[[ -f "$import_file" ]] || fail 'Fichier privé du catalogue et des destinataires manquant dans le ZIP.'
python -c 'import json,pathlib,sys;p=pathlib.Path(sys.argv[1]);d=json.loads(p.read_text());assert d["format"]=="journal-cr-v15.1" and len(p.read_bytes())<45000' "$import_file" || fail 'Import privé invalide.'
...
if [[ "$(prepare changed)" == yes ]]; then
  BACKUP_DIR="$(mktemp -d "${JOURNAL_BACKUP_BASE:-$HOME}/Journal-Chantier-V15.1-sauvegarde-XXXXXXXX")"
...
PHASE='preparation du catalogue et des suggestions de destinataires'
gh secret set JOURNAL_CR_V151_IMPORT --repo "$REPOSITORY" < "$import_file"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.1 - serveur des horaires par perimetre'
  git_remote push origin main
...
echo 'Attente : tests des droits, migration V15.1 et configuration des notifications...'
python -u "$RELEASE_ROOT/scripts/wait-v151-workflow.py" "$backend_sha" "${wait_options[@]}"
...
if ! git diff --cached --quiet; then
  git commit -m 'Journal Chantier V15.1 - CR off terrain et destinataires'
  git_remote push origin main
fi
PHASE='terminee'
echo 'Pour le premier envoi email, consulter VERSION_V15.1.md (adresse expediteur a configurer).'
echo 'V15.1 envoyee : horaires par perimetre et liste de destinataires a verifier.'
...
echo 'Ouvrez CR off, preparez une nuit puis cochez les perimetres attendus. Choisissez les destinataires avant validation.'
