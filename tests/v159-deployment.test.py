...
spec = importlib.util.spec_from_file_location('release', ROOT / 'scripts/deploy-v159-release.py')
...
class DeploymentTests(unittest.TestCase):
    ...
    def test_invalid_package_fails_before_any_connection(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(m, 'ROOT', root), mock.patch.object(m.briefing.transport, 'postgres_environment') as connect:
                with self.assertRaisesRegex(RuntimeError, 'Fichier SQL V15.9 invalide.*aucune migration exécutée'):
                    m.main()
    ...
    def test_print_transaction_without_credentials(self):
        ...
        result = subprocess.run([sys.executable, str(ROOT / 'scripts/deploy-v159-release.py'), '--print-sql'],
