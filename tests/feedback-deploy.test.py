...
spec=importlib.util.spec_from_file_location('prepare_mode_test', SOURCE/'scripts/prepare-feedback-update.py')
prepare=importlib.util.module_from_spec(spec)
spec.loader.exec_module(prepare)


class FeedbackInstallTests(unittest.TestCase):
    def setUp(self):
        ...
        self.old_config=(self.repo/'supabase/config.toml').read_bytes()
...
if 'push' in a:
 ...
 if os.environ.get('MODE_FAIL_PUSH')=='all' or (os.environ.get('MODE_FAIL_PUSH')=='frontend' and message.endswith('fil des ameliorations application')):
...
if a[:2]==['auth','status']:pass
 ...
 record={'id':ident,'head_sha':head,'head_branch':'main','event':'push','path':'.github/workflows/deploy-feedback.yml@main','status':'completed','conclusion':'failure' if os.environ.get('MODE_BACKEND_FAIL')=='1' else 'success'}
 ...
 elif '/contents/' in route:print(json.dumps({'path':'.github/workflows/deploy-feedback.yml'}))
...
''')
    ...
    def install(self,success=True,**env):
        result=subprocess.run(['bash',str(self.payload/'scripts/update-feedback-termux.sh'),str(self.repo)],cwd=self.base,
    ...
    def test_v142_upgrade_is_backend_first_and_idempotent(self):
        ...
        self.preserved()
        self.assertTrue(list(self.base.glob('Journal-Chantier-retours-V14.5-sauvegarde-*/code-avant.tar')))
    ...
    def test_v143_optional_visual_baseline_is_accepted(self):
        ...
        self.install()
        self.preserved()
        for name in prepare.PATCHED:
            self.assertEqual((self.repo/name).read_bytes(),self.patched(name,V143))

    @unittest.skipUnless(V144, 'V14.4 baseline required for recovery')
    def test_v144_backend_with_old_interface_stays_independent(self):
        old_ui={name:(self.repo/name).read_bytes() for name in prepare.PATCHED}
        for parent in ('scripts','supabase','.github'):
            for source in (V144/parent).rglob('*'):
                if source.is_file() and '__pycache__' not in source.parts:
                    target=self.repo/source.relative_to(V144)
                    target.parent.mkdir(parents=True,exist_ok=True);target.write_bytes(source.read_bytes())
        old_mode=(self.repo/'scripts/configure-mode-chantier.py').read_bytes()
        self.git('add','.');self.git('commit','-m','Partially installed V14.4 backend')
        self.git('push','origin','main')
        self.install(MODE_FORBID_NAME_LOOKUP='1')
        for name,raw in old_ui.items():self.assertEqual((self.repo/name).read_bytes(),self.patch(name,raw))
        self.assertFalse((self.repo/'mode-chantier.js').exists())
        self.assertEqual((self.repo/'scripts/configure-mode-chantier.py').read_bytes(),old_mode)
        self.preserved()

    @unittest.skipUnless(V144, 'V14.4 baseline required')
    def test_complete_v144_keeps_mode_chantier(self):
        for parent in ('scripts','supabase','.github'):
            for source in (V144/parent).rglob('*'):
                if source.is_file() and '__pycache__' not in source.parts:
                    target=self.repo/source.relative_to(V144)
                    target.parent.mkdir(parents=True,exist_ok=True);target.write_bytes(source.read_bytes())
        for name in ('app-v13.js','index.html','service-worker-v13.js','mode-chantier.js','mode-chantier.css','styles-v14.3.css'):
            shutil.copyfile(V144/name,self.repo/name)
        self.git('add','.');self.git('commit','-m','Complete V14.4');self.git('push','origin','main')
        self.install()
        for name in prepare.PATCHED:self.assertEqual((self.repo/name).read_bytes(),self.patched(name,V144))
        self.assertEqual((self.repo/'mode-chantier.js').read_bytes(),(V144/'mode-chantier.js').read_bytes())
        self.preserved()
    ...
    def test_backend_failure_keeps_previous_interface_and_prints_diagnostic(self):
        ...
        self.assertFalse((self.repo/'feedback.js').exists())
        self.assertIn('DIAGNOSTIC AUTOMATIQUE',output)
        self.install()
        self.assertEqual((self.repo/'index.html').read_bytes(),self.patched('index.html', V142))
        self.preserved()
    ...
    def test_additional_supabase_config_is_preserved(self):
        ...
        self.assertEqual(data.count('[functions.journal-mode-push]'),0)

    def test_old_workflows_do_not_match_feedback_payload(self):
        import fnmatch
        for workflow_name in ('deploy-supabase-migrations.yml','deploy-mode-chantier.yml'):
            old=(SOURCE/'.github/workflows'/workflow_name).read_text()
            paths=[];inside=False
            ...
            self.assertFalse([(name,pattern) for name in prepare.BACKEND+prepare.FRONTEND for pattern in paths if fnmatch.fnmatchcase(name,pattern)])

    def patch(self,name,raw):
        spec=importlib.util.spec_from_file_location('feedback_patch_for_tests',SOURCE/'scripts/feedback-ui-patch.py')
        module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
        return module.patch_file(name,raw)

    def patched(self,name,base):return self.patch(name,(base/name).read_bytes())
