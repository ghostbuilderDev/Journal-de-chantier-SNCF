...
spec = importlib.util.spec_from_file_location('release', ROOT / 'scripts/deploy-v154-release.py')
...
class DeploymentTests(unittest.TestCase):
    ...
    def test_invalid_package_fails_before_any_connection(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(m, 'ROOT', root), mock.patch.object(m.briefing.transport, 'postgres_environment') as connect:
                with self.assertRaisesRegex(RuntimeError, 'Fichier SQL V15.4 invalide.*aucune migration exécutée'):
                    m.main()
    ...
    def test_print_transaction_without_credentials(self):
        ...
        result = subprocess.run([sys.executable, str(ROOT / 'scripts/deploy-v154-release.py'), '--print-sql'],
    ...
    def check_resume(self, failed_function=False):
        ...
        cli_results = [SimpleNamespace(returncode=1 if failed_function else 0), SimpleNamespace(returncode=0)]
        with mock.patch.dict(os.environ, ENV), \
             mock.patch.object(m.briefing.transport, 'postgres_query', side_effect=query), \
             mock.patch.object(m.subprocess, 'run', side_effect=cli_results) as cli, \
             mock.patch.object(m.urllib.request, 'urlopen', side_effect=HTTPError('https://test.invalid', 401, 'Unauthorized', {}, None)), \
             contextlib.redirect_stdout(io.StringIO()):
            if failed_function:
                with self.assertRaisesRegex(RuntimeError, 'service email interrompu'):
                    m.main()
            else:
                m.main()
            m.main()
    ...
    def test_resume_after_function_failure_does_not_replay_sql(self):
        self.check_resume(failed_function=True)
