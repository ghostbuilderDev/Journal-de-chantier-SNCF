...
const root=path.resolve(__dirname,'..'),out=process.env.TEST_OUTPUT_DIR||'/tmp/journal-v15101-signature-tests';fs.mkdirSync(out,{recursive:true});
(async()=>{const db=new PGlite();let browser,server;
 try{
  ...
  const people=await frame.evaluate(()=>BriefingPresence.list());assert.deepEqual(people.filter(p=>p.remote_id).map(p=>p.nom),['MARTIN','DURAND']);assert.ok(people.filter(p=>p.remote_id).every(p=>p.entreprise==='ENTREPRISE TEST'));
  ...
  fs.writeFileSync(path.join(out,'briefing-qr-v15101.pdf'),Buffer.from(pdf.bytes));assert.equal(Buffer.from(pdf.bytes).subarray(0,5).toString(),'%PDF-');assert.ok(pdf.bytes.length>20000);
