...
(async()=>{let browser,server;try{
 ...
 assert.ok(cached.some(u=>u.endsWith('signature-pad.js?v=15.10.1')));assert.ok(cached.some(u=>u.endsWith('/rapport/app.js?v=15.10.5')));assert.ok(cached.some(u=>u.endsWith('/rapport/collaboration.js?v=15.10.5')));assert.ok(cached.some(u=>u.endsWith('/rapport/report-actions.js?v=15.10.5')));assert.ok(cached.some(u=>u.endsWith('/rapport/collaboration.css?v=15.10.5')));assert.equal(cached.some(u=>u.includes('/rest/')),false);
 assert.equal(cached.some(u=>u.includes('presence-review.')),false);assert.ok(cached.some(u=>u.endsWith('presence-matching.js?v=15.10.3')));
 ...
 console.log('PASS V15.10.5 PWA: root/briefing/report workers and static assets install, private requests not cached, drafts retained, QR URL and correct offline page preserved.');
