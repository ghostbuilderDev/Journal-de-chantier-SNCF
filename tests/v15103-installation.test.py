...
spec=importlib.util.spec_from_file_location('installer',ROOT/'scripts/prepare-v15103-update.py')
...
class Tests(unittest.TestCase):
 ...
 def test_interrupted_v15103_release_only_accepts_known_bytes(self):
  previous,new=b'paquet V15.10 interrompu',b'paquet corrige'
 ...
 def test_phases(self):
  ...
  self.assertIn('briefing/presence-review.js',m.FRONTEND)
  self.assertIn('briefing/briefing-attendance.js',m.FRONTEND)
  self.assertEqual(m.BACKEND,[])
  self.assertFalse(any(f.startswith('supabase/') for f in m.FRONTEND))
  self.assertIn('briefing/presence-matching.js',m.FRONTEND)
  self.assertNotIn('config.js',m.BACKEND+m.FRONTEND)
  self.assertFalse(any('donnees-privees' in f for f in m.BACKEND+m.FRONTEND))
 def test_no_database_deployment_or_secret_request(self):
  shell=(ROOT/'scripts/update-v15103-termux.sh').read_text()
  self.assertNotIn('gh secret',shell)
  self.assertNotIn('deploy-v1510',shell)
  self.assertNotIn('wait-v1510',shell)
  self.assertIn(m.MESSAGES['frontend'],shell)
